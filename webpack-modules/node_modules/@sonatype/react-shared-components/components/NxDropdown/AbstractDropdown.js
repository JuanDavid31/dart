 ...
 */
var AbstractDropdown = forwardRef(function (props, ref) {
    var className = props.className, isOpen = props.isOpen, disabled = props.disabled, renderToggleElement = props.renderToggleElement, children = props.children, onToggleCollapseProp = props.onToggleCollapse, onKeyDownProp = props.onKeyDown, onCloseClick = props.onCloseClick, onCloseKeyDownProp = props.onCloseKeyDown, menuRefProp = props.menuRef, attrs = __rest(props, ["className", "isOpen", "disabled", "renderToggleElement", "children", "onToggleCollapse", "onKeyDown", "onCloseClick", "onCloseKeyDown", "menuRef"]);
    var menuRef = useRef(null), menuRefs = __spreadArray([menuRef], (menuRefProp ? [menuRefProp] : []), true), mergedMenuRef = useMergedRef.apply(void 0, menuRefs), toggleRef = useRef(null);
    ...
    var handleDocumentClick = function (event) {
        if (isOpen && onToggleCollapseProp) {
            ...
            if (onCloseClick) {
                ...
                */
                var proxiedClickEvent = new Proxy(event, {
                    get: function (target, prop) {
                        if (prop === 'preventDefault') {
                            return function () { defaultPrevented_1 = true; };
                        }
                        else {
    ...
    function onMenuClosing() {
        var focusedEl = document.activeElement, menuEl = menuRef.current, toggleEl = toggleRef.current;
        if (menuEl && menuEl.contains(focusedEl)) {
            toggleEl === null || toggleEl === void 0 ? void 0 : toggleEl.focus();
        }
    }
    return (
    // Ignore the a11y linter, this onKeyDown is to catch events bubbling up from interactive children
    /* eslint-disable-next-line jsx-a11y/no-static-element-interactions */
    React.createElement("div", __assign({ ref: ref, className: className, onKeyDown: onKeyDown }, attrs),
        renderToggleElement(toggleRef, onToggleCollapse),
        isOpen &&
            React.createElement(NxDropdownMenu, { ref: mergedMenuRef, onClosing: onMenuClosing }, children)));
});
